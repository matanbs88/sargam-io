...
import { mockSong, rootOptions, type MockSong } from "@/src/lib/mockTranscription";
...
export default function Home() {
  ...
  const [song, setSong] = useState<MockSong>(mockSong);

  const notes = useMemo(() => formatRelativeNotes(song.midiNotes, rootMidi, notation), [notation, rootMidi, song]);
  ...
  async function confirmSettings() {
    ...
    try {
      ...
      const result = await response.json() as { song: MockSong };
      setSong(result.song);
  ...
  function downloadNotes() {
    ...
    link.download = `${song.title.toLowerCase().replaceAll(" ", "-")}-${notation}.txt`;
    link.click();
    URL.revokeObjectURL(downloadUrl);
  }

  if (stage === "results") {
    return <ResultsView song={song} notation={notation} setNotation={setNotation} groupedNotes={groupedNotes} rootMidi={rootMidi} setRootMidi={setRootMidi} instrument={instrument} bansuriKey={bansuriKey} copied={copied} onCopy={copyNotes} onDownload={downloadNotes} onStartOver={() => setStage("home")} />;
  }
